...
<?php 
if(isset($_SESSION['email']) != ''){ //sudah dalam keadaan login
...
if(isset($_POST['simpan'])){
    ...
    if(empty($err)){
        ...
        kirim_email($id_users,$email,$nama_lengkap,$judul_email,$isi_email);

        $sql1       = "insert into users(id_user,email,nama,password,role) values ($id_users,'$email','$nama_lengkap','$password','$status')";
        $q1         = mysqli_query($koneksi,$sql1);
        if($q1){
...
<form action="" method="POST">
    <table>
    <tr>
            <td class="label">ID Test Aja wakgeng</td>
            <td>
                <input type="text" name="id_users" class="input" value="<?php echo $id_users?>"/>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                <input type="text" name="nama" class="input" value="<?php echo $nama_lengkap?>"/>
            </td>
        </tr>
        <tr>
